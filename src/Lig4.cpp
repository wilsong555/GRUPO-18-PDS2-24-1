...
bool Lig4::jogar(int coluna, std::string valor) {
    ...
    for (int linha = 0; linha < this->get_linhas(); ++linha) {
        for (int coluna = 0; coluna < this->get_colunas(); ++coluna) {
            if (this->getvalor_matriz(linha, coluna) != " ") {
                ...
                if (coluna <= this->get_colunas() - 4 && verifica_direcao(linha, coluna, 0, 1)) {
                    this->set_status('v');
                    return;
                }
                // Verificação Vertical
                if (linha <= this->get_linhas() - 4 && verifica_direcao(linha, coluna, 1, 0)) {
                    this->set_status('v');
                    return;
                }
                // Verificação Diagonal Crescente
                if (linha <= this->get_linhas() - 4 && coluna <= this->get_colunas() - 4 && verifica_direcao(linha, coluna, 1, 1)) {
                    this->set_status('v');
                    return;
                }
                // Verificação Diagonal Decrescente
                if (linha >= 3 && coluna <= this->get_colunas() - 4 && verifica_direcao(linha, coluna, -1, 1)) {
                    this->set_status('v');
                    return;
                }
            }
